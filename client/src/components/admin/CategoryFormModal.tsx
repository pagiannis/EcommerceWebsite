...
import type { CategoryItem, CategoryPayload } from "../../services/adminCategoriesService";
...
export default function CategoryFormModal({ category, onClose }: Props) {
  ...
  async function onSubmit(values: FormValues) {
    try {
      if (isEdit) {
        await update.mutateAsync({ id: category.id, payload: toPayload(values) });
      } else {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="px-6 py-5 space-y-4 overflow-y-auto flex-1">
            <div>
              ...
              <input {...register("name")} className={inputCls} placeholder="e.g. Men" />
              {errors.name && (
